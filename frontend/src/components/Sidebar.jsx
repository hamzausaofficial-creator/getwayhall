...
const Sidebar = ({ isCollapsed, setIsCollapsed, isMobile, isMobileOpen, onMobileClose }) => {
  ...
  const hallNavItems = [
    ...
    ...(canAccessPayments ? [{ name: 'Payments', icon: CreditCard, path: '/payments' }] : []),
  ...
  const guestHouseNavItems = [
    ...
    ...(canAccessPayments ? [{ name: 'Payments', icon: CreditCard, path: '/gh/payments', pageKey: GH_PAGE_KEYS.PAYMENTS }] : []),
