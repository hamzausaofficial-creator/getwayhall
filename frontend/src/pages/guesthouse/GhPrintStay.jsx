...
export default function GhPrintStay() {
  ...
  const remaining = Number(stay.remaining_balance) || 0;
  ...
  return (
    <GhPrintShell
      title={activeDoc === 'advance' ? 'Advance receipt' : 'Stay invoice'}
        ...
        {DOC_TABS.map((tab) => (
          <button
            ...
            style={{
              ...
              padding: '8px 14px',
              borderRadius: '8px',
              border: activeDoc === tab.id ? '2px solid #5BD51E' : '1px solid #e2e8f0',
              background: activeDoc === tab.id ? '#f0fdf4' : '#fff',
              fontWeight: '700',
              fontSize: '12px',
              cursor: 'pointer',
            }}
          >
            {tab.id === 'advance' ? <DollarSign size={14} /> : <FileText size={14} />}
            {tab.label}
          </button>
        ))}
      </div>

      <GhPrintHeader docType={activeDoc === 'advance' ? 'Advance receipt / ایڈوانس رسیڈ' : 'Stay booking invoice'} />
